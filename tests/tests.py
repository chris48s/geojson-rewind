...
class RewindTests(unittest.TestCase):
    ...
    def test_feature_collection(self):
        ...
        self.assertDictEqual(rewind(json.load(open(infile))), json.load(open(outfile)))
    ...
    def test_passthrough(self):
        ...
        self.assertDictEqual(rewind(json.load(open(infile))), json.load(open(outfile)))
        self.assertDictEqual(rewind(json.load(open(infile))), json.load(open(infile)))
    ...
    def test_near_zero_area(self):
        infile = self.get_fixture_path("near_zero.input.geojson")
        outfile = self.get_fixture_path("near_zero.output.geojson")
        self.assertDictEqual(rewind(json.load(open(infile))), json.load(open(outfile)))

    def test_empty(self):
        infile = self.get_fixture_path("empty.input.geojson")
        outfile = self.get_fixture_path("empty.output.geojson")
        self.assertDictEqual(rewind(json.load(open(infile))), json.load(open(outfile)))
    ...
    def test_unexpected_crs_warning(self):
        ...
        with io.StringIO() as buf, redirect_stderr(buf):
            rewind(_input)
            self.assertIn(
                "Co-ordinates in the input data are assumed to be WGS84", buf.getvalue()
            )
    ...
    def test_cli_stdin_is_tty(self):
        with patch("sys.stdin.isatty", return_value=True):
            with io.StringIO() as buf, redirect_stdout(buf):
                self.assertEqual(0, main())
                self.assertIn(
                    "Enforce RFC 7946 ring winding order", buf.getvalue().strip()
                )
